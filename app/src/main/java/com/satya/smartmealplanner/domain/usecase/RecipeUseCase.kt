...
class RecipeUseCase @Inject constructor(
    ...
    suspend fun getRecipeDetailsById(recipeId: Int): Resource<SelectedRecipeDetails> =
        repository.getRecipeDetailsById(recipeId)
    ...
    suspend fun getRandomJoke(forceRefresh: Boolean): Resource<RandomJoke?> {
        return if (forceRefresh) {
            ...
            val savedJoke: Resource<FoodFactEntity> =
                repository.getRandomJokeFromDb(type = Constants.JOKE)
    ...
    suspend fun getRandomTrivia(forceRefresh: Boolean): Resource<FoodTrivia?> {
        return if (forceRefresh) {
            ...
            val savedTrivia: Resource<FoodFactEntity> =
                repository.getRandomTriviaFromDb(type = Constants.TRIVIA)
    ...
    suspend fun fetchRecipesByQuery(searchQuery: String, isVeg: Boolean): Resource<SearchByQuery?> =
    ...
    suspend fun fetchSimilarRecipesById(recipeId: Int) =
        repository.fetchSimilarRecipesById(recipeId)
